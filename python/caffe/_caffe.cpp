...
class CaffeBlob {
 public:
  ...
  int depth() const {return blob_->depth();}
...
class CaffeBlobWrap : public CaffeBlob {
 public:
  ...
  object get_data() {
      npy_intp dims[] = {num(), channels(), height(), width(), depth()};
...
struct CaffeNet {
  ...
  inline void check_contiguous_array(PyArrayObject* arr, string name,
      int channels, int height, int width, int depth) {
	...
	if (PyArray_DIMS(arr)[4] != depth) {
      throw std::runtime_error(name + " has wrong depth");
    }
  ...
  void set_input_arrays(object data_obj, object labels_obj) {
    ...
    check_contiguous_array(data_arr, "data array", md_layer->datum_channels(),
        md_layer->datum_height(), md_layer->datum_width(),md_layer->datum_depth() );
...
BOOST_PYTHON_MODULE(_caffe) {
  ...
  boost::python::class_<CaffeBlob, CaffeBlobWrap>(
	  ...
	  .add_property("depth",    &CaffeBlob::depth)
