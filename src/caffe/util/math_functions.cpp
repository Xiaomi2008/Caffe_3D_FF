...
template <typename Dtype>
void caffe_set(const size_t N, const size_t alpha, size_t* Y) {
  if (alpha == 0) {
    memset(Y, 0, sizeof(Dtype) * N);
    return;
  }
...
//template void caffe_set<size_t>(const size_t N, const double alpha, size_t* Y);
