...
void LRNLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  case LRNParameter_NormRegion_ACROSS_CHANNELS:
    ...
    scale_.Reshape(num_, channels_, height_, width_ ,  depth_);
    break;
  case LRNParameter_NormRegion_WITHIN_CHANNEL:
    {
      ...
      CHECK_EQ(square_output_.width(), width_);
	   CHECK_EQ(square_output_.depth(), depth_);
	  ...
	  CHECK_EQ(pool_output_.depth(), depth_);
	  ...
	  CHECK_EQ(power_output_.depth(), depth_);
...
Dtype LRNLayer<Dtype>::CrossChannelForward_cpu(
  ...
  Blob<Dtype> padded_square(1, channels_ + size_ - 1, height_ , width_, depth_);
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    caffe_sqr(channels_ * height_ * width_ *depth_,
        bottom_data + bottom[0]->offset(n),
        padded_square_data + padded_square.offset(0, pre_pad_));
    // Create the first channel scale
    for (int c = 0; c < size_; ++c) {
      caffe_axpy<Dtype>(height_ * width_* depth_, alpha_over_size,
          padded_square_data + padded_square.offset(0, c),
          scale_data + scale_.offset(n, 0));
    }
    for (int c = 1; c < channels_; ++c) {
      // copy previous scale
      caffe_copy<Dtype>(height_ * width_ *depth_,
...
void LRNLayer<Dtype>::CrossChannelBackward_cpu(
  ...
  Blob<Dtype> padded_ratio(1, channels_ + size_ - 1, height_, width_, depth_);
  Blob<Dtype> accum_ratio(1, 1, height_, width_, depth_);
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    for (int c = 0; c < channels_; ++c) {
      ...
      caffe_axpy<Dtype>(height_ * width_*depth_, -cache_ratio_value,
