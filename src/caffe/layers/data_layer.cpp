...
void* DataLayerPrefetch(void* layer_pointer) {
  ...
  //LOG(INFO)<<"current data size is: "<< size;
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    if (crop_size) {
      ...
      if (layer->phase_ == Caffe::TRAIN) {
		//  h_off = layer->PrefetchRand() % ((height - crop_size)/2) ;//+(height - crop_size)/4;
		//  w_off = layer->PrefetchRand() % ((width - crop_size)/2) ;//+(width - crop_size)/4;
		//	d_off = layer->PrefetchRand() % ((depth - crop_size)/2) ;//+ (width - crop_size)/4;
	...
	//for (int item_id = 0; item_id < batch_size; ++item_id){
        ...
        for (int j = 0; j < size; ++j){
		   //Dtype datum_element =
            //  static_cast<Dtype>(static_cast<uint8_t>(data[j]));
          if (top_data[0 * size + j] != top_data[(batch_size-1) * size + j]){
		      is_same=false;
			  break;
		  }
