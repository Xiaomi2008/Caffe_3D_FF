...
void DummyDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK(param.width_size() == 1 || param.width_size() == num_top)
   ...
   CHECK(param.depth_size() == 1 || param.depth_size() == num_top)
      << "Must specify either a single (1) 'depth' or one for each top blob "
      << "(" << num_top << "); you specified " << param.depth_size() << ".";
  ...
  for (int i = 0; i < num_top; ++i) {
	...
	const int depth =
	    (param.depth_size() == 1) ? param.depth(0) : param.depth(i);
    (*top)[i]->Reshape(num, channels, height, width, depth);
  }
