		  ...
		  if sub_dir_struct(j).isdir
				...
				for k=1:length(files_names)
				    ...
				    if sum(strfind(file, 'WM'))>0 && strcmp(ext,'.hdr')
                        ...
                        pet_idx=find(strcmp(f_names,'WarpedPETt1ToMdl_smooth_downsample.hdr'));
                        
                        info = analyze75info([cur_dir filesep f_names{pet_idx}]);
                        element_labels{count}=analyze75read(info);
                         count=count+1;
					end
