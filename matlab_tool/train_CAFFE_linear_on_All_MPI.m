function train_CAFFE_linear_on_All_MPI(Age,layer,mat_file,featuresampleMod,fineTune)
...
% end
if ~fineTune
	ovf_mex_file=fullfile(data_dir,[mat_file Age '_L_' layer '.mat']);
else
	ovf_mex_file=fullfile(data_dir,[mat_file Age '_L_' layer '_FineTune.mat']);
end
saveDir=translatePath('Z:\ABA\autoGenelable_multi_lables_proj\result');
filePartMod='whole';
%featuresampleMod ='Max';
if ~fineTune
saveFile =[saveDir  filesep 'classResultMIP_' Age '_L',num2str(layer) '_' filePartMod '_' featuresampleMod '.mat'];
else
saveFile =[saveDir  filesep 'classResultMIP_' Age '_L',num2str(layer) '_' filePartMod '_' featuresampleMod 'FineTune.mat'];
end


if ~exist(ovf_mex_file,'file')
    disp(['File : ' ovf_mex_file ' not exist quit ...'])
    return;
else
    load(ovf_mex_file);
end
...
%end
	...
	if exist(result_file,'file')==2
		...
		if ~fineTune
			saveFilePrefix =[saveDir filesep 'classfied_CAFFE_' filePartMod '_' featuresampleMod  '_Matrix_' Age  '_L',num2str(layer)];
		else
			saveFilePrefix =[saveDir filesep 'classfied_CAFFE_' filePartMod '_' featuresampleMod  '_Matrix_' Age  '_L',num2str(layer) '_FineTune'];
		end
		disp(['result file saved to ' saveFile ]);
		convertMPI_TrainResult_byOntology(saveFile, saveFilePrefix)
		generateAUCReportOnCAFFE(Age,layer,filePartMod,featuresampleMod);
	else
