...
int main(int argc, char** argv) {
  ...
  sum_blob.set_depth(datum.depth());
  
  LOG(ERROR) <<"Datum size  = " << datum.channels() <<" "<<datum.height() <<" "<< datum.width() << " " <<datum.depth();
  const int data_size = datum.channels() * datum.height() * datum.width() *datum.depth();
  ...
  //-----------------------------------------------
  // testing if all image all the same pic
  if (db_backend == "leveldb"){
     ...
     datum2.ParseFromString(it->value().ToString());
	 
	 
	  const string& data_1 = datum.data();
	  const string& data_2 = datum2.data();
	  bool is_same =true;
	  if (data_1.size() != 0) {
        for (int i = 0; i < size_in_datum; ++i) {
          if (((uint8_t)data_1[i]) != (uint8_t)data_2[i])
		  {
				LOG(ERROR)<<"data are not same ...";
				is_same =false;
				break;
		  }
        }   
		   if (is_same){
		    LOG(ERROR)<<"data are the SAME !!!! ...";
			}
      } else {
        for (int i = 0; i < size_in_datum; ++i) {
             if( static_cast<float>(datum.float_data(i)) != static_cast<float>(datum2.float_data(i)))
			 {
			    LOG(ERROR)<<"data are not same ...";
				is_same =false;
				break;
