...
//   [depth, width, height, channels, images]
...
static mxArray* blob_data_to_mxarray(const Blob<float>* blob) {
  mwSize dims[5] = {blob->depth(), blob->width(), blob->height(),
                    blob->channels(), blob->num()};
  DLOG(INFO) << dims[0] << " " << dims[1] << " " << dims[2] << " " << dims[3]<<" " << dims[4];
  mxArray* mx_blob_data = mxCreateNumericArray(5, dims, mxSINGLE_CLASS, mxREAL);
...
static void mxarray_to_blob_data(const mxArray* data, Blob<float>* blob) {
  mwSize dims[5] = {blob->depth(), blob->width(), blob->height(),
                    blob->channels(), blob->num()};
  LOG(INFO) << dims[0] << " " << dims[1] << " " << dims[2] << " " << dims[3] <<" "<<dims[4];
...
static void mxarray_to_blob_diff(const mxArray* data, Blob<float>* blob) {
  ...
  LOG(INFO) << dims[0] << " " << dims[1] << " " << dims[2] << " " << dims[3] << " " << dims[4];
   //CHECK_EQ_MEX(blob->count(), mxGetNumberOfElements(data),
   // "blob->count() don't match with numel(data)");
	LOG(INFO)<<"const data_ptr =reinterpret_cast<const float* const>(mxGetPr(data)" << "blobs size = " << blob->count();
  const float* const data_ptr =
      reinterpret_cast<const float* const>(mxGetPr(data));
  switch (Caffe::mode()) {
  case Caffe::CPU:
    LOG(INFO)<<"CPU   caffe_copy(blob->count(), data_ptr, blob->mutable_gpu_diff())";
    caffe_copy(blob->count(), data_ptr, blob->mutable_cpu_diff());
    break;
  case Caffe::GPU:
    LOG(INFO)<<"GPU caffe_copy(blob->count(), data_ptr, blob->mutable_gpu_diff())";
...
static void do_set_input_blobs(const mxArray* const bottom) {
  ...
  for (unsigned int i = 0; i < input_blobs.size(); ++i) {
   LOG(INFO)<<" start const mxArray* const elem = mxGetCell(bottom, i);";
    const mxArray* const elem = mxGetCell(bottom, i);
	LOG(INFO)<<" const mxArray* const elem = mxGetCell(bottom, i);";
...
static mxArray* do_forward(const mxArray* const bottom, mxArray* mx_loss) {
  ...
  LOG(INFO)<<"do forward_ prefilled";
...
static mxArray* do_get_layers_info() {
  ...
  {
    mxArray* mx_blob;
    const char* blobfnames[6] = {"num", "channels", "height", "width", "depth", "count"};
    for (unsigned int i = 0; i < layers.size(); ++i) {
      ...
      mx_blob = mxCreateStructArray(1, num_blobs, 6, blobfnames);
      ...
      for (unsigned int j = 0; j < layer_blobs.size(); ++j) {
        ...
        mxSetField(mx_blob, j, "width",
		 ...
		 mxSetField(mx_blob, j, "depth",
          mxCreateDoubleScalar(layer_blobs[j]->depth()));
...
static mxArray* do_get_blobs_info() {
  ...
  mxArray* mx_blobs;
  {
    ...
    const char* fnames[7] = {"name", "num", "channels",
      "height", "width", "depth", "count"};
    mx_blobs = mxCreateStructArray(1, num_blobs, 7, fnames);
  }
  ...
  // Step 2: copy info into output
  {
    for (unsigned int i = 0; i < blobs.size(); ++i) {
	  ...
	  mxSetField(mx_blobs, i, "depth",
        mxCreateDoubleScalar(blobs[i]->depth()));
...
static mxArray* do_get_all_data() {
  ...
  for (unsigned int i = 0; i < blobs.size(); ++i) {
    DLOG(INFO) << blob_names[i];
    mwSize dims[5] = {blobs[i]->depth(),blobs[i]->width(), blobs[i]->height(),
...
static void read_mean(MEX_ARGS) {
    ...
    mwSize dims[5] = {data_mean.depth(), data_mean.width(), data_mean.height(),
                      data_mean.channels(), data_mean.num() };
    mxArray* mx_blob =  mxCreateNumericArray(5, dims, mxSINGLE_CLASS, mxREAL);
